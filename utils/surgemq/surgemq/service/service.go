...
func (this *service) deviceAuth(mqttclientID, mqttpassword string) error {
	//log.Debugf("service | deviceAuth mqttclientID:%s productKey:%s deviceName:%s mqttpassword:%s",mqttclientID,this.productKey,this.deviceName,mqttpassword)
	//err := rpc.Auth(mqttclientID, this.productKey, this.deviceName, mqttpassword)
	//if err != nil {
	//	log.Errorf("service.deviceAuth | Auth error:%s", err.Error())
	//	return message.ErrAuthServerAuthFailed
	//}
	return nil
}
func (this *service) deviceLogin() error {
	cid := fmt.Sprintf("%s:%d", this.nodeID, this.id)
	err := rpc.Login(this.productKey, this.deviceName, cid)
	if err != nil {
		log.Errorf("service.deviceLogin | Login error:%s", err.Error())
		return message.ErrAuthServerLoginFailed
	}
	return nil
}
func (this *service) deviceLogout() error {
	err := rpc.Logout(this.productKey, this.deviceName, this.nodeID)
	if err != nil {
		log.Errorf("service.deviceLogout | Logout error:%s", err.Error())
		return message.ErrAuthServerLogoutFailed
	}
	return nil
}
