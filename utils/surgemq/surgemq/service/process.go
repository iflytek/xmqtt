...
func (this *service) processPublish(msg *message.PublishMessage) error {
	...
	if string(msg.Topic()) == "/sys/service/heartbeat/update" {
		...
		if err != nil {
			log.Debugf("processPublish | update heartbeat err:%s", err.Error())
			return err
		}
		...
		// 更新心跳
		log.Debugf("processPublish | update heartbeat new:%d", heartbeat)
		//atomic.StoreInt32(&this.newKeepAlive, int32(heartbeat))
		// 注意阻塞
...
func (this *service) onPublish(msg *message.PublishMessage) error {
	log.Debugf("service.onPublish | len(msg):%d\n", len(msg.Payload()))
	if msg.Retain() {
	...
	log.Debugf("service.onPublish | len(subs):%d\n", len(this.subs))
	msg.SetRetain(false)
	...
	for i, s := range this.subs {
		if s != nil {
			...
			if !ok {
				...
				// 已msg和sub最小qos发消息 by haojin 2018-5-14
				//msg.SetQoS(this.qoss[i])
				log.Debugf("service.onPublish2 | i:%d len(msg):%d\n", i, len(msg.Payload()))
				(*fn)(msg)
			}
		}
	}
