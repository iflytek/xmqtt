...
import (
	...
	"git.iflytek.com/HY_XIoT/core/xfyun/surgemq/surgemq/auth"
	"git.iflytek.com/HY_XIoT/core/xfyun/surgemq/surgemq/rpc/auth"
...
// 设置rpc Server地址
func (this *Server) SetRPCServer(addrs string) {
	this.authServers = addrs
}
...
func (this *Server) Init() error {
	// 创建并初始化shadow Clinet
	//this.shadowCli = rpc.NewShadowClient(this.shadowServer)
	//return this.shadowCli.Init()
	this.svcMap = make(map[string]*service)
	return rpc.Init(this.authServers)
}
...
func (this *Server) Close() error {
	...
	for cid, svc := range this.svcMap {
		log.Infof("Stopping service %d, clientID:%s", svc.id, cid)
		svc.stop()
	}
...
func (this *Server) handleConnection(c io.Closer) (svc *service, err error) {
	...
	if err != nil {
		log.Errorf("server | handleConnection getConnectMessage err:%s", err.Error())
	...
	if Username != "" && strings.Contains(Username, "&") {
		productKey = strings.Split(Username, "&")[1]
		deviceName = strings.Split(Username, "&")[0]
	} else {
	...
	/*if err = this.authMgr.Authenticate(Username, Password); err != nil {
	...
	// 当clientID == inner 时跳过登陆
	if string(req.ClientId()) != "inner" {
		// 新增Login逻辑,携带productID、deviceID
		// haojin 2018-5-15
		/*
			err = this.shadowCli.DoLogin(Username, ClientId, this.nodeID)
			if err != nil {
				resp.SetReturnCode(message.ErrShadowLoginFailed)
				resp.SetSessionPresent(false)
				writeMessage(conn, resp)
				return nil, err
			}
		*/
		//新增Auth签名校验逻辑，需携带ClientId、UserName、PassWord
		// leiyuan 2018-8-3
		log.Debugf("server | handleConnection req.ClientId:%s req.Username:%s req.Password:%s", ClientId, Username, Password)

		err := svc.deviceAuth(ClientId, Password)

		if err != nil {
			//resp.SetReturnCode(message.ErrAuthServerAuthFailed)
			resp.SetSessionPresent(false)
			writeMessage(conn, resp)
			return nil, err
		}
	...
	// 相同cid，后面连接的将之前连接的剔除下线
	this.mu.Lock()
	svcOld, ok := this.svcMap[svc.deviceName+"&"+svc.productKey]
	if ok {
		// 剔除老连接
		log.Infof("Server.handleConnection | Tick off old did:%s connection", svc.deviceName)
		svcOld.stop()
		delete(this.svcMap, svc.deviceName+"&"+svc.productKey)
	}
	// 添加新连接
	this.svcMap[svc.deviceName+"&"+svc.productKey] = svc
	this.mu.Unlock()
