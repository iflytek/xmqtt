...
func (r *timeoutReader) Read(b []byte) (int, error) {
	//log.Infof("Read | r.d:%p d:%d", &r.d, r.d)
	heartbeat := time.Second * time.Duration(atomic.LoadInt64(&r.d))
	//log.Infof("timeoutReader.Read | %d", heartbeat/time.Second)
...
func (this *service) receiver() {
	...
	case net.Conn:
		log.Debugf("server/handleConnection: Setting read deadline to %d(s)", this.keepAlive)
		heartbeat := time.Second * time.Duration(this.keepAlive)
		if err := conn.SetReadDeadline(time.Now().Add(heartbeat)); err != nil {
			log.Errorf("server.handleConnection | SetReadDeadline err:%s", err.Error())
			return
		}

		// 延迟3秒关闭
		keepAlive := int64(this.keepAlive + 3)
		r := timeoutReader{
			d:    keepAlive,
			conn: conn,
		}
...
func (this *service) updateHeartbeat(r *timeoutReader, keepAlive int32) {
	defer func() {
		...
		log.Debugf("(%s) Stopping updateHeartbeater", this.cid())
	}()
	...
	for {
		...
		case newKeepAlive = <-this.keepAliveCH:
			if atomic.CompareAndSwapInt32(&old, old, newKeepAlive) {
				log.Infof("updateHeartbeat | heartbeat old:%d new:%d", old, newKeepAlive)
				temp := int64(newKeepAlive + 3)
				log.Infof("updateHeartbeat | keepalive:%d", temp)
				atomic.StoreInt64(&r.d, temp)
				log.Infof("updateHeartbeat | r.d:%p", &r.d)
				heartbeat := time.Second * time.Duration(temp)
				if err := r.conn.SetReadDeadline(time.Now().Add(heartbeat)); err != nil {
					log.Errorf("updateHeartbeat | SetReadDeadline err:%s", err.Error())
				}
			}
		case <-this.done:
			return
