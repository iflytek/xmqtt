...
func (this *ConnectMessage) SetClientId(v []byte) error {
	//modified by leiyuan 2018-08-08
	//The MQTT protocol Client ID can't be longer than 23 characters,
	// but it's not a standard MQTT protocol,
	// so this check is temporarily discarded
	...
	if !strings.ContainsAny(string(v), "|securemode&algorithm&timeStamp") {
		return ErrIdentifierRejected
	}
...
func (this *ConnectMessage) decodeMessage(src []byte) (int, error) {
	...
	// refer to mqtt3.11 if usrname is set no need passwd set, but if usrname not set passwd must not set
	// change by haojin 2017-5-14
	if !this.UsernameFlag() && this.PasswordFlag() {
		return total, fmt.Errorf("connect/decodeMessage: Username flag is not set but Password flag is set, invalid")
...
func (this *ConnectMessage) validClientId(cid []byte) bool {
	...
	//modified by leiyuan 2018-08-09
	//The MQTT protocol Client ID can't be longer than 23 characters,
	// but it's not a standard MQTT protocol,
	// so this check is temporarily discarded
	...
	return strings.ContainsAny(string(cid), "|securemode&algorithm&timeStamp")
}
